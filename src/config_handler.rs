...
impl ConfigObject {
    fn new() -> ConfigObject {
        return ConfigObject {
            valuetype: ConfigValues::Empty,
            value: ConfigValue::Nullus,
        };
    }
    ...
    fn from(value: String) -> Self {
        if value.starts_with("\"") && value.ends_with("\"") {
            ...
            chars.next_back();
            return ConfigObject {
                valuetype: ConfigValues::string,
                value: ConfigValue::string(chars.as_str().to_string()),
            };
            ...
            return ConfigObject {
                valuetype: ConfigValues::tuple,
                value: ConfigValue::tuple(tuple),
            };
        } else if value.starts_with("[") && value.ends_with("]") {
            ...
            return ConfigObject {
                valuetype: ConfigValues::list,
                value: ConfigValue::list(vector),
            };
        } else {
            ...
            if tmp.len() > 0 {
                while tmp[0].is_numeric() && tmp.len() > 0 {
                    ...
                    if tmp.len() == 0 {
                        return ConfigObject {
                            valuetype: ConfigValues::int,
                            value: ConfigValue::int(value.parse::<i32>().unwrap()),
                        };
                    }
                }
            }
...
impl Config {
    ...
    pub fn to_string(&mut self) -> String {
        ...
        for i in 0..(self.keys.len()) {
            result +=
                &(self.keys[i.clone()].clone() + "=" + &(self.values[i].value.clone().as_str()));
    ...
    for i in string.split(";") {
        config.push(
            i.split("=").collect::<Vec<&str>>()[0]
                ...
                .replace("\n", "")
                .replace(" ", ""),
            ConfigObject::from(fix_string(
                i.split("=").collect::<Vec<&str>>()[1].to_string(),
            )),
        );
    }
    return config;
